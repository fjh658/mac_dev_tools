...
INSTRUMENTS_APP_PATH="/Applications/Instruments_16.app"
...
if [ ! -d "${DEV_PRIV_FX}/CoreDeviceUtilities.framework" ]; then
    mkdir -p "${DEV_PRIV_FX}"
    cp -Rf "${INSTRUMENTS_APP_PATH}/Contents/Frameworks/CoreDeviceUtilities.framework" "${DEV_PRIV_FX}"    
fi
